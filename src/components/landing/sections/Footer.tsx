      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12 mb-12">
          ...
          <div className="lg:col-span-4 pr-0 lg:pr-8">
            <Link href="/" className="flex items-center gap-2 mb-6">
              ...
              <span className="text-xl font-bold text-white tracking-tight">{settings.COMPANY_NAME}</span>
            </Link>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="space-y-4">
              <a href={`https://t.me/${settings.TELEGRAM_SUPPORT_BOT}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-slate-300 hover:text-white transition-colors group">
              ...
              <a href={`mailto:${settings.SUPPORT_EMAIL}`} className="flex items-center gap-3 text-sm text-slate-300 hover:text-white transition-colors group">
        ...
        <div className="pt-8 border-t border-slate-800/50 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="text-xs text-muted-foreground space-y-1.5">
            <p><span className="text-slate-300 font-medium">Реквизиты:</span> {settings.COMPANY_NAME}</p>
            <p>ИНН: {settings.COMPANY_INN} | ОГРНИП: {settings.COMPANY_OGRNIP}</p>
            {settings.COMPANY_ADDRESS && <p>Адрес: {settings.COMPANY_ADDRESS}</p>}
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/30">
          ...
          <p className="text-xs text-muted-foreground">
            © {new Date().getFullYear()} {settings.COMPANY_NAME}. Все права защищены.
          </p>
        </div>
      </div>
    </footer>
