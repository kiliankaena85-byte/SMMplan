...
- **Внедрение Link-First логики и умной фильтрации в Telegram-боте (100% COMPLETE & LIVE VERIFIED):**
  ...
  - **Верификация:** `npx tsc --noEmit` — 0 ошибок. Тесты `src/__tests__/bot-order-real-execution.test.ts` (3/3 pass), скрипт `scripts/test-bot-catalog-invariants.ts` подтвердил 100% сокрытие пустых сетей. Бандл `dist/bot.js` пересобран, контейнер `smmplan_bot` перезапущен. Коммиты: `7156e80f`, `a93c4f68`.
