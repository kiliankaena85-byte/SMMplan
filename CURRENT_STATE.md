...
- **Внедрение Link-First логики и умной фильтрации в Telegram-боте (100% COMPLETE & LIVE VERIFIED):**
  ...
  - **Верификация:** `npx tsc --noEmit` — 0 ошибок. Тесты в `src/__tests__/bot-order-real-execution.test.ts` (3/3 pass, включая проверку фильтрации категорий под канал). Бандл `dist/bot.js` собран заново, образ пересобран и контейнер `smmplan_bot` перезапущен в Docker. Коммит: `7156e80f`.
