...
> **Последнее обновление:** 2026-09-01 07:32 (МСК)
...
- **Dedicated Transactions Tab, Fullscreen Ledger Modal & Orders Filter Optimization (100% COMPLETE & VERIFIED):**
  ...
  - **3. Устранение бага с черным списком и оптимизация выпадающих списков на странице «Заказы» (`OrdersFilterForm.tsx`):**
    - Устранено появление черного нестилизованного списка и мерцание при открытии селекторов сетей, категорий, статусов и поставщиков.
    - Внедрена явная стилизация `<select>` и `<option>` токенами темы (`bg-card text-foreground dark:bg-slate-900 dark:text-slate-100`).
  ...
  - **4. Верификация:**
    ...
    - `vitest` $\rightarrow$ **39/39 файлов (316/316 PASS, 100% GREEN)**.
    - Puppeteer E2E на Stage 3005: скриншоты `36` (чистая карточка), `37` (модал Ledger), `38` (модал платежей), `39` (реестр `/admin/transactions`), `40` (оптимизированные фильтры заказов).
