...
- [x] `npx tsc --noEmit` — 0 ошибок (Strict TypeScript PASS).
- [x] `npm run check:arch` — 0 архитектурных нарушений (Level 3 -> Level 2 -> Level 1 -> Level 0).
- [x] `npx vitest run` — 13/13 тестов PASS (Sanitizer, Key Pool Rotation, UI Decomposition).

---

## 10. Детальный UX-анализ и спецификация взаимодействия (Fullstack & UX Architecture)

### 10.1. Где виджет появляется и как его запускать? (Placement & Triggers)

Для предотвращения визуального шума и случайных кликов в OmniSMM 1.0 реализована **многоуровневая система точек входа (Tiered Invocation)**:

1. **Точка входа №1 (Основная в UI): Иконка в Header (Верхняя панель)**
   - **Размещение**: в шапке панели (`<header>`) справа, перед меню профиля администратора `<AdminProfileDropdown />`.
   - **Вид**: компактная кнопка с иконкой `Sparkles` / `BookOpen`, текстовым бейджем `Справка и ИИ`, индикатором статуса удаленной памяти (зеленая/янтарная точка) и подсказкой шортката `Ctrl + /`.
   - **Плюс**: верхняя панель зафиксирована (`sticky top-0`) и всегда видна оператору независимо от скролла таблицы заказов или каталога. Она **на 100% свободна от элементов форм** и никогда ничего не перекрывает.

2. **Точка входа №2 (Клавиатурная / Профессиональная): Глобальный хоткей**
   - Шорткат **`Ctrl + /`** (для Windows/Linux) и **`Cmd + /`** (для macOS).
   - Работает из любого места админки. Нажатие мгновенно разворачивает виджет с автофокусом в поле ввода вопроса.
   - Повторное нажатие `Ctrl + /` или клавиши `Escape` моментально сворачивает виджет.

3. **Точка входа №3 (Палитра команд): Интеграция в CommandPalette (`Ctrl + K`)**
   - При открытии палитры команд оператор видит секцию `Интеллектуальный помощник`:
     * `Задать вопрос ИИ-консультанту...`
     * `Открыть интерактивную инструкцию по текущей странице`
     * `Открыть регламент: Настройка эквайринга 54-ФЗ`

4. **Точка входа №4 (Плавающий FAB — опциональный)**:
   - Кнопка в правом нижнем углу экрана (`bottom-4 right-4`).
   - По умолчанию аккуратно свернута и смещена выше пагинации таблиц, чтобы не перекрывать кнопки действий в футере. В настройках интерфейса оператор может отключить плавающую кнопку, оставив только иконку в Header и шорткат.

---

### 10.2. Защита от перекрытия функционала (Zero-Overlap & Side-by-Side Docking Invariant)

Главный риск стандартных чат-ботов — **перекрытие рабочей области**: оператор не может одновременно читать инструкцию и нажимать кнопки в таблице заказов или заполнять форму провайдера.

В OmniManual 1.0 внедрена **двойная механика отображения (Dual Display Architecture)**:

```
┌─────────────────────────────────────────────────────────────┬───────────────────────────┐
│ OmniSMM Header: [Логотип] [Свитчер тенантов] [Режимы]       │ [✨ Справка и ИИ] [Профиль]│
├─────────────────────────────────────────────────────────────┼───────────────────────────┤
│                                                             │ OmniManual 1.0 (460px)    │
│  РАБОЧАЯ ОБЛАСТЬ АДМИНКИ (Main Content)                     │ [Чат] [Гайды] [Инспектор] │
│  • Форма импорта каталога                                   ├───────────────────────────┤
│  • Таблица заказов                                          │ Шаг 1: Выберите провайдера│
│  • Настройки шлюзов ЮKassa                                  │ [x] Шаг 2: Нажмите кнопку │
│  • Кнопки действий, пагинация, модалки                      │     "Синхронизировать"    │
│                                                             │ [ ] Шаг 3: Задайте наценку│
│  (Прикрепленный режим: контент сжимается без перекрытия)    │                           │
└─────────────────────────────────────────────────────────────┴───────────────────────────┘
```

1. **Режим 1: Overlay Drawer (Всплывающий справа)**:
   - Ширина: 460px на десктопе, полупрозрачный блюр-бэкдроп.
   - Используется для быстрого вопроса-ответа («Как расшифровать код ошибки?») за 10 секунд.
2. **Режим 2: Side-by-Side Docking («Работать параллельно»)**:
   - В шапке виджета размещена иконка `[📌 Закрепить панель сбоку]`.
   - При клике виджет превращается в стационарную правую колонку (460px), а основная рабочая область `main-content` плавно сжимается через CSS: `w-[calc(100%-460px)]`.
   - **Результат**: Ни один пиксель таблицы, формы или кнопок сохранения **НЕ ПЕРЕКРЫВАЕТСЯ**. Оператор видит шаги инструкции справа и одновременно вводит данные в форму слева!
3. **Мобильные экраны (< 768px)**:
   - Автоматически трансформируется в Bottom Sheet (шторка снизу на 85% высоты экрана) с возможностью свайпа вниз для быстрого закрытия.

---

### 10.3. Как вообще работать с виджетом? (Операционные сценарии)

Виджет разделен на 3 специализированных инструмента под разные задачи оператора:

#### Сценарий А: Возник нестандартный вопрос или ошибка $\to$ Вкладка «AI-Консультант»
1. Нажимаем `Ctrl + /`.
2. Виджет автоматически определяет, где находится оператор (например: `/admin/orders`), и подсвечивает чип: `Контекст: Заказы`.
3. Оператор пишет вопрос естественным языком: *«Почему заказ #18423 завис в статусе ERROR и как вернуть деньги клиенту?»*.
4. Gemini 3.8 Flash запрашивает удаленную векторную память, находит логику `WalletOps.refund()` и `failOrderTerminal()`, и за 1 секунду выдает пошаговый ответ со ссылкой на заказ и кнопку возврата.

#### Сценарий Б: Выполнение типовой регламентной задачи $\to$ Вкладка «Инструкция и Гайды»
1. Оператор впервые настраивает эквайринг ЮKassa или импортирует каталог провайдера.
2. Открывает вкладку «Инструкция».
3. Вверху списка автоматически закреплен регламент именно для открытого раздела (с бейджем `Для этой страницы`).
4. Оператор открывает гайд и видит интерактивный чеклист:
   - [x] Шаг 1: Проверить реквизиты магазина.
   - [ ] Шаг 2: Указать ставку НДС 22% по 425-ФЗ.
   - [ ] Шаг 3: Провести тестовый платеж в режиме `ACQUIRING_TEST`.
5. Клик по чекбоксу сохраняет прогресс в `localStorage`. В каждом шаге встроена прямая кнопка-ссылка (например, `[Перейти к настройкам эквайринга]`).

#### Сценарий В: Аудит архитектуры и разработческий контроль $\to$ Вкладка «Инспектор»
1. Оператору или тимлиду нужно сверить модель БД Prisma или узнать, какой ADR регламентирует работу с балансом.
2. Во вкладке «Инспектор» доступен мгновенный фильтр по `ADR`, `MODEL`, `SECURITY` с прямыми ссылками на исходный код.

---

### 10.4. Как задавать вопросы и получать ответы?

1. **Контекстные подсказки в 1 клик (Zero-Typing Prompts)**:
   - В зависимости от текущего URL над полем ввода отображаются 3 быстрых чипса:
     * В `/admin/providers`: `[Как работает Cherry-Pick?]`, `[Что такое теневой каталог?]`, `[Как настроить наценку?]`.
     * В `/admin/finance`: `[Как настроить НДС 22%?]`, `[Как работает BigInt копейки?]`, `[Как сверить баланс?]`.
     * В `/admin/orders`: `[Как отменить заказ?]`, `[Почему статус ERROR?]`, `[Что такое Drip-Feed Floor?]`.
   - Достаточно одного клика мыши, чтобы отправить вопрос без набора текста.
2. **Формат ввода**:
   - Многострочное поле с авто-высотой (`min-h-[36px] max-h-[80px]`).
   - `Enter` — мгновенная отправка. `Shift + Enter` — перенос строки.
3. **Формат ответа (Streaming & Deep Links)**:
   - **Стриминг**: текст выводится плавно по мере генерации Gemini 3.8 Flash (Server-Sent Events).
   - **Цитаты кода**: внизу ответа выводятся бейджи файлов, на основе которых сформирован ответ (например: `src/services/financial/exact-math.ts`).
   - **Глубокие ссылки (Deep Links)**: в тексте ответа ссылки на экраны админки (`[Перейти в Настройки](/admin/settings)`) кликабельны и работают через клиентский роутинг Next.js без перезагрузки страницы.
   - **Кнопка копирования**: в один клик копирует готовые сниппеты параметров, формулы или фрагменты конфигураций.

---

### 10.5. Как читать документацию и регламенты?

1. **Единый рубрикатор (8 глав)**: вся документация платформы оцифрована в 8 ключевых глав с фильтрацией по соцсетям, платежам, провайдерам и аварийным ситуациям.
2. **Мгновенный живой поиск**: строка поиска фильтрует статьи по названию, тегам и тексту описания на лету.
3. **Оценка времени прохождения**: у каждого гайда указано расчетное время выполнения (например, `~5 мин`), что позволяет оператору планировать работу.
4. **Защита от потери прогресса**: отмеченные шаги чек-листов сохраняются локально в браузере оператора. При повторном открытии прогресс не сбрасывается.
