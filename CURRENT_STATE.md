...
## Активный статус: 🚀 Успешный пуш в origin/main. Завершена реализация Anti-Fraud контура для реферальной системы и бонусов: графовая проверка циклов (Cycle & Self-Referral Ban), детекция IP-кластеризации, модель ProcessedBonusEvent с уникальностью (eventType, eventId), проверка дубликатов платежных фингерпринтов First Deposit, система заморозки бонусов (Vesting 72h) с досрочным одобрением и конфискацией, защита промокодов от брутфорса, админ-панель Anti-Fraud Monitor (/admin/fraud-monitor), тесты 51/51 passed.
