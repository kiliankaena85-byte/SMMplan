...
3. **[UX-CATALOG-FILTER-PERSIST] Сохранение фильтров каталога при возврате после редактирования услуги:**
   - **Проблема:** При сохранении или отмене редактирования услуги на странице `/admin/catalog/[id]` происходит редирект на базовый `/admin/catalog` с полным сбросом всех активных фильтров оператора (соцсеть, категория, поисковый запрос, статус активности, провайдер, номер страницы). Оператору приходится повторно настраивать весь срез каталога.
   - **Решение:**
     - Передавать строку поисковых параметров каталога (`returnQuery` / `returnUrl`) при переходе к редактированию услуги: `/admin/catalog/[id]?returnUrl=...`.
     - В `ServiceEditForm` при нажатии «Сохранить» или «Отмена» возвращать оператора по сохранённому `returnUrl` (или с восстановлением query-параметров из `searchParams`).
     - Сохранять параметры пагинации и поиска в URL каталога, чтобы состояние фильтрации не терялось.
