...
1. **[LEGAL-001] Правки оферты v3.0 (Статус: DONE)**
   - **Что сделано:** Внесены 3 критические правки от юридического департамента:
     - П. 3.1: зафиксирован субподряд по ст. 706 ГК РФ и условие «не является агентским договором».
...
2. **[LEGAL-007] Fallback для всех юридических страниц (Статус: DONE)**
   - **Что сделано:** Реализована отказоустойчивая 4-этапная система генерации контента в `LegalPageContent.tsx`:
     1. Запрос в БД `ContentItem` для активной страницы.
     2. В случае сбоя БД или отсутствия записи — загрузка статического fallback-контента из `src/data/legal-fallbacks.ts` через `getLegalFallback(slug)`.
     3. Вызов `notFound()` при абсолютном отсутствии контента.
     4. Автоматическая замена всех переменный тегов (`{{COMPANY_NAME}}`, `{{COMPANY_INN}}`, `{{SUPPORT_EMAIL}}` и др.) через `SettingsProvider`.
