...
export function GeminiSettings({ settings, tenantId = 'smmplan' }: { settings: SystemSettings; tenantId?: string }) {
  const [testing, setTesting] = useState(false);
  
  const handleTest = async () => {
    setTesting(true);
    try {
      const res = await testGeminiAiConnectionAction(undefined, undefined, tenantId);
