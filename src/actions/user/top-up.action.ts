  ...
  try {
    ...
    if ((gateway === 'yookassa' || gateway === 'sbp' || gateway === 'robokassa') && amountCents > 1_500_000) {
      if (!dbUser.telegramId) {
        return {
          success: false,
          error: "Для пополнения баланса свыше 15 000 ₽ картой или СБП, пожалуйста, привяжите ваш Telegram-аккаунт в настройках профиля либо воспользуйтесь безналичным расчетом для юрлиц (B2B)."
        };
      }
    }
