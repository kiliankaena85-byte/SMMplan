...
- **WFM Support Schedule Calendar, Zero-Scroll Transactions & RBAC CSV Security (100% COMPLETE & VERIFIED):**
  - **1. Интерактивный Календарь смен и Расписание саппорта (OmniSMM WFM Calendar — `/admin/staff`):**
    - **3 режима отображения:**
      1. 📅 **«Сетка месяца» (Month Grid 7x5)**: классический календарь с цветовыми чипами (☀️ Дневная `09:00–21:00`, 🌙 Ночная `21:00–09:00`, 🔄 Подмена, 🌴 Отпуск, 🩹 Больничный, ☕ Отгул), подсветкой сегодняшнего дня и быстрыми кнопками назначения.
      2. 📋 **«Расписание недели» (Week Timetable Matrix Grid)**: академическая матрица расписания (по строкам: ☀️ Дневная `09:00–21:00`, 🌙 Ночная `21:00–09:00`, 🌴 Отсутствия; по колонкам: 7 дней недели с датами и индикатором «СЕГОДНЯ»), компактными чипами операторов с аватарами и быстрыми кнопками `+ Назначить`.
      3. 👤 **«Мой график» (My Shifts)**: персональный дашборд оператора (план смен, отработанные часы, подмены, начисления) с возможностью в 1 клик передать смену коллеге (`ShiftSwapModal`) или оформить отпуск/больничный (`TimeOffModal`).
    - **Контроль покрытия 24/7 (Coverage Guard)**: плашка в реальном времени отображает % укомплектованности смен и подсвечивает слоты, оставшиеся без дежурного оператора.
    - **Биржа подмен саппорта (Peer Shift Swap)**: операторы могут легко меняться сменами или передавать дежурство коллегам с фиксацией часов и примечаний.
  - **2. Пагинация и 100% Zero Horizontal Scroll в таблице транзакций (`/admin/transactions`):**
    - Внедрена пагинация `NumberedPagination` (как на странице Заказов) с номерами страниц, стрелками и переключателем размера страницы `20` / `50` / `100`.
    - Добавлена колонка **UUID / Шлюз ID** с кнопкой копирования в 1 клик идентификаторов YooKassa (`yoo:...`), номеров заказов (`order:...`) и внутренних проводок.
    - Таблица полностью избавлена от горизонтального скролла (`hasTableOverflow: false`, `hasBodyOverflow: false`).
  - **3. Строгая защита от утечек данных (Zero Data Exfiltration for Support):**
    - Кнопки выгрузки CSV на странице транзакций (`/admin/transactions`) и в табеле зарплат (`/admin/staff`) **полностью скрыты для роли `SUPPORT`** и доступны исключительно `OWNER` и `ADMIN`.
  - **4. Верификация:**
    - `npx tsc --noEmit` $\rightarrow$ **0 ошибок**.
    - `vitest` $\rightarrow$ **40/40 файлов (322/322 PASS, 100% GREEN)**.
    - Puppeteer E2E на Stage 3005: скриншоты `43`, `44`, `45`, `46`, `47`, `48`.
