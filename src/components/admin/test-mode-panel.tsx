...
export function TestModePanel({ initialIsTestMode }: TestModePanelProps) {
  ...
  return (
    <div className={`rounded-xl border-2 p-5 transition-all duration-300 ${
      isTestMode 
        ? 'border-amber-400 bg-amber-50/80 shadow-amber-100 shadow-lg' 
      ...
      {message && (
        <div className="mt-3 text-xs font-medium text-slate-600 bg-white/80 rounded-lg px-3 py-2 border border-slate-200">
          ✅ {message}
        </div>
      )}
    </div>
