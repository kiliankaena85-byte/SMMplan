...
## 2.5. 🛑 MANDATORY CONTAINER REBUILD & DEPLOYMENT GATE (CRITICAL RULE)
- **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** пересобирать или перезапускать Docker-контейнеры без предварительного явного подтверждения пользователя.
- Перед предложением пересборки агент ОБЯЗАН:
  1. Описать подробно: что сделано, почему обновляем, что изменится.
  2. Проверить отсутствие утечки секретов и персональных данных.
  3. Проверить Git: создать осмысленный коммит и выполнить `git push` в GitHub.
