...
- **Финансовая целостность:**
  - Все изменения баланса — ТОЛЬКО через `WalletOps`. Все суммы строго в `BigInt` (копейки). Все аудиты — через `await auditAdminAwaitable()`.

---

## 2. 🛑 Специфика компонентов и решенные антипаттерны
