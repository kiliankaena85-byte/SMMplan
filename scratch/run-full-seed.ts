...
async function main() {
  console.log('--- STARTING FULL SEED VERIFICATION ---');
  ...
  // 1. Providers
  console.log('Creating/upserting providers...');
  ...
  console.log(`Loaded ${providers.length} providers from DB`);

  // 2. Networks & Categories
  console.log('Creating networks and categories...');
  ...
  for (let i = 0; i < networks.length; i++) {
    ...
    console.log(`Created Network: "${name}" (id: ${nw.id})`);
  ...
  for (const cat of createdCategories) {
    ...
    for (let t = 0; t < tiers.length; t++) {
      ...
      console.log(`  -> Inserting service tier "${tiers[t]}" for "${cat.name}" using Category ID: "${cat.id}"`);
  ...
  console.log('Generating 150 mock orders...');
  ...
  console.log('--- ALL SEEDING AND VERIFICATIONS COMPLETED SUCCESSFULLY! 🎉 ---');
}

main()
  .catch(e => {
    console.error('Fatal Error during full seed:', e);
  })
