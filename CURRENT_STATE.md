...
- **Внедрение Link-First логики и умной фильтрации в Telegram-боте (100% COMPLETE & LIVE VERIFIED):**
  ...
  - **Авто-очистка тестовых заказов & Защита от ложных алертов:** Зачищены все 12 тестовых заказов (#315–#326) из базы PostgreSQL и 34 зависших джоба из очередей BullMQ/Redis, вызывавших ложные сбои мокового провайдера. В интеграционные тесты (`src/__tests__/bot-order-real-execution.test.ts`) внедрён хук `afterAll`, который гарантированно удаляет созданные тестовые заказы и очищает очередь сразу после завершения теста.
  - **Верификация:** `npx tsc --noEmit` — 0 ошибок. Тесты `src/__tests__/bot-order-real-execution.test.ts` (3/3 pass), скрипт `scripts/test-bot-catalog-invariants.ts` подтвердил 100% сокрытие пустых сетей. Бандл `dist/bot.js` пересобран, контейнер `smmplan_bot` перезапущен. Коммиты: `7156e80f`, `a93c4f68`, `d5a1fe13`.
