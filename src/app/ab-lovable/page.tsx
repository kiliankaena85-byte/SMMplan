...
export async function generateMetadata() {
  ...
  return {
    ...
    alternates: {
      canonical: absoluteCanonical(tenantId, '/'),
    },
    robots: {
      index: false,
      follow: false,
...
export default async function FluxShowcasePage() {
  permanentRedirect('/');

  const catalogResult = await getPublicCatalogAction();
  const catalog: FluxNetwork[] = (catalogResult.success && catalogResult.data ? catalogResult.data : []) as unknown as FluxNetwork[];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans flex flex-col relative overflow-x-clip">
      ...
      <div className="relative z-10 w-full">
        <Header siteName={siteName} activePath="/" />
      </div>
