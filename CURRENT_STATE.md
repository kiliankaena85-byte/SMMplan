...
- **Сквозная верификация заказа в Telegram-боте (100% COMPLETE & LIVE VERIFIED):**
  - **Пересборка и актуализация контейнера бота (`smmplan_bot`):** Собран свежий production-бандл `dist/bot.js` (`npm run build:bot`) и пересобран контейнер `smmplan_bot` с новыми правилами классификации `CHANNEL_POSTS` и свежим курсом ЦБ.
  - **Устранение ошибок в боте:** Проверено, что ни `SYSTEM_HALT: Currency exchange rate is older than 48 hours`, ни `LINK_SERVICE_MISMATCH: применяется к конкретным записям` больше не возникают при заказе на `https://t.me/smmMarket69`.
  - **Сквозной интеграционный тест (`src/__tests__/bot-order-real-execution.test.ts`):** 
    - Протестирован `orderService.createOrder` от имени реального Telegram-пользователя (`telegramId: 1382446520`).
    - Проверена финтех-проводка: атомарное создание `LedgerEntry` со списанием `-500` копеек (`ORDER_CHARGE`), запись заказа в PostgreSQL (`Order` со статусом `PENDING`, сумма 5.00 ₽, 100 шт, ссылка `https://t.me/smmMarket69`).
    - Тесты добавлены в `vitest.unit.config.ts` и пройдены на 100% (2/2 pass, коммит `4d9819ef`).
