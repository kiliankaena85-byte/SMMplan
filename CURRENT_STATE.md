- [x] ⚡ [PROJECT-AUDIT-REMEDIATION-2026] Комплексное устранение дефектов сквозного аудита (100% COMPLETE & PASS):
  * 🛡️ **[GATE-01] Ликвидация блокеров Production Readiness (`audit:prod`):**
    ...
    - В `src/actions/admin/telegram-bot.ts` сырой `console.log` заменен на структурированный `logger.info(...)`.
  ...
  * 🗄️ **[DB-01] Создание недостающих индексов внешних ключей (`prisma/schema.prisma`):**
    ...
    - Индексы сгенерированы в клиенте Prisma (`prisma generate`) и применены в СУБД PostgreSQL (`smmplan_lite_db`).
...
- [x] ⚡ [CATALOG-CATEGORY-SWITCH-UX-STABILIZATION-2026] Ликвидация мерцающих скелетонов и стабилизация смены категорий каталога (100% COMPLETE & LIVE):
  * 🎯 **Паттерн Stale-While-Revalidate в `useOrderEngine.ts`:**
    ...
    - Карточки текущей категории сохраняются на экране с плавной полупрозрачностью `opacity-60` и блокировкой повторных кликов до прибытия новых данных.
    - В заголовок витрины добавлен аккуратный спиннер `Loader2` («Обновление...»).
    - На мобильных устройствах (`MobileStep3Service.tsx`) скелетоны отображаются только при холодном старте, когда список услуг абсолютно пуст.
  * 🌐 **Стабилизация маршрутизации туннеля (`docker-compose.yml`):**
    - В переменную `ALLOWED_ORIGINS` сервиса `web` добавлен домен туннеля `https://smmplan.tailbb9d28.ts.net`, гарантируя мгновенную валидацию Server Actions.
  * 🧪 **Верификация & Тесты:**
    - `npx tsc --noEmit` — 0 ошибок (PASS).
    - `npm test` — 21/21 тестов PASS (100%).
    - Standalone сборка Next.js 16 (`npm run build`) прошла без ошибок, 0 утечек секретов.
    - Контейнер `smmplan_web` пересобран и перезапущен: healthcheck 200 OK.
    - Проверка через внешний туннель `https://smmplan.tailbb9d28.ts.net/` — Server Action возвращает 200 OK со всеми 38 услугами без падений и задержек.
