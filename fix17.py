...
# Replace providerName line
content = re.sub(
    r"<span className=\"text-sm text-foreground\">\{order\.providerName([^<]+)</span>",
    r"""<div className="flex flex-col">
                <span className="text-sm text-foreground">{order.providerName\1</span>
                {order.externalId ? (
...
charge_div_regex = r"(<span className=\"text-sm font-bold text-foreground\">\{chargeRub\.toFixed\(2\)\}[^<]+</span>\s*</div>)"
...
with open('src/components/admin/order-details/OrderMinimalSummary.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated OrderMinimalSummary via flexible regex")
