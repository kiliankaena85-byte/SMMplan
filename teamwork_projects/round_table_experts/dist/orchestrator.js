...
exports.RoundTableOrchestrator = void 0;
...
class RoundTableOrchestrator {
    ...
    constructor(config) {
        this.config = {
            logPath: config?.logPath || (0, path_1.join)(process.cwd(), "DISCUSSION_LOG.json"),
    ...
    async callLLM(expert, systemPrompt, userMessage, schema) {
    ...
    async execute(requestId, requestPayload) {
        const turns = [];
        const factsIngested = [];
        ...
        while (!architectApproved && correctionAttempts < maxCorrectionAttempts) {
            ...
            const architectOutput = await this.callLLM("Architect", architectPrompt, architectInput, types_1.ArchitectOutputSchema);
            turns.push({
                expert: "Architect",
            ...
            const securityOutput = await this.callLLM("SecurityAuditor", securityPrompt, proposal, types_1.SecurityAuditorOutputSchema);
            turns.push({
        ...
        if (architectApproved) {
            ...
            const qaOutput = await this.callLLM("QAEngineer", qaPrompt, proposal, types_1.QAEngineerOutputSchema);
            turns.push({
                expert: "QAEngineer",
                input: proposal,
                output: qaOutput,
        ...
        if (hasKnowledgeGap) {
            ...
            const researcherOutput = await this.callLLM("DeepResearcher", researcherPrompt, requestPayload, types_1.DeepResearcherOutputSchema);
            turns.push({
        ...
        (0, fs_1.writeFileSync)(this.config.logPath, JSON.stringify(log, null, 2), "utf8");
        return log;
    }
}
exports.RoundTableOrchestrator = RoundTableOrchestrator;
exports.default = RoundTableOrchestrator;
