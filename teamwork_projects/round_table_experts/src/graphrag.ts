...
export async function ingest(
  ...
  const uniqueSources = Array.from(
    new Set(sources.map((s) => s.trim().toLowerCase()).filter(Boolean))
  );
 ...
 */
export function compressContext(messages: string[], maxLimit: number): string {
  ...
  const filtered = messages
    ...
    .filter((msg) => {
      ...
      if (boilerplateRegex.test(msg)) return false;
      return true;
    });
  ...
  const summaries = filtered.map((msg) => {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        const parts: string[] = [];
        if (parsed.proposal) parts.push(`Proposal: ${parsed.proposal}`);
        if (parsed.architectureSummary) parts.push(`Summary: ${parsed.architectureSummary}`);
        if (parsed.vulnerabilities && Array.isArray(parsed.vulnerabilities)) {
          parts.push(`Vulnerabilities: ${parsed.vulnerabilities.join(", ")}`);
        }
        if (parsed.securityFeedback) parts.push(`Security: ${parsed.securityFeedback}`);
        if (parsed.qaFeedback) parts.push(`QA: ${parsed.qaFeedback}`);
        if (parsed.findings) parts.push(`Findings: ${parsed.findings}`);
        if (parts.length > 0) {
          return parts.join(" | ");
        }
      }
...
export class GraphRAGClient {
  ...
  static compressContext(messages: string[], maxLimit: number): string {
