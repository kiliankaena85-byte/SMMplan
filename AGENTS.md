...
## 0.5. 🛑 MANDATORY CONTAINER REBUILD & DEPLOYMENT GATE (CRITICAL RULE)
> ⚠️ **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО пересобирать или перезапускать Docker-контейнеры без предварительного явного подтверждения пользователя.**
> Перед предложением пересборки агент ОБЯЗАН предоставить подробный отчет:
> 1. **Что конкретно изменено:** список файлов и архитектурных изменений.
> 2. **Зачем нужно обновление:** точная бизнес-/техническая причина.
> 3. **Что изменится для пользователей:** влияние на витрины, админку и фоновые очереди.
